...
class Converter {
	...
	async createOutputFolder(arrayOfOutputFolder, sourcePath, targetPath) {
		...
		for (file of arrayOfOutputFolder)
			if (!fs.existsSync(file)) {// kiểm tra xem ở đường dẫn có thư mục đó chưa ?
				...
				let mkdirChild = await exec(`cd "${path.dirname(file)}" && mkdir "${path.basename(file)}"`)
	...
	async convertFolder(bitRate, arrayOfInputFlacs, arrayOfOutputFlacs) { //eg: 128k
		if (arrayOfInputFlacs.length === arrayOfOutputFlacs.length) {
			// dùng async mapseries để lặp tuần tự

			await Promise.all(arrayOfInputFlacs.map(async (inputFlac) => {
				...
				let ffmpeg = await exec(`ffmpeg -y -i "${inputFlac}" -ab ${bitRate} -map_metadata 0 -id3v2_version 3 "${arrayOfOutputFlacs[i].replace('.flac', '.mp3')}" `)
...
const flacToMp3 = (bitRate,inFile, outFile) => {
	return new Promise((resolve, reject) => {
		const ffmpeg = exec(`ffmpeg -y -i "${inFile}" -ab ${bitRate} -map_metadata 0 -id3v2_version 3 "${outFile.replace('.flac', '.mp3')}"`)
...
const loop = async (bitRate, arrayOfInputFlacs, arrayOfOutputFlacs) => {
	for(let inFlac of arrayOfInputFlacs){
		let i = arrayOfInputFlacs.indexOf(inFlac)
		const ffmpeg = await flacToMp3(bitRate,inFlac,arrayOfOutputFlacs[i].replace('.flac','.mp3'))
	}
}

loop('128k', info.fileData.arrOfInputFlacs, info.fileData.arrOfOutputFlacs)
